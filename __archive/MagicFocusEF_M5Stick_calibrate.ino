...
// for Hardware Serial Communication to LIDAR
// for ESP32
//#define RX2_PIN 16
//#define TX2_PIN 17
// M5 Stick
...
void setup()
{
    ...
    M5.begin();  // activate the M5 Stick
    //M5.begin(true,true,true);  // activate the M5 Stick
    
    /*  hardware serial setup for LIDAR sensor */
    //          baud rate  protocol
    //              v         v
    Serial2.begin(115200, SERIAL_8N1, RX2_PIN, TX2_PIN);  // Serial for LIDAR reading. Using M5Stick, baudrate of 921600 cannot be achieved.
    delay(10);
...
//    digitalWrite(REC_LED,LOW);
  ...
  //M5.Axp.ScreenBreath(10);  // lower the screen brightness a bit. This could be dynamically adjusted using  
  M5.Lcd.setRotation(3);
  M5.Lcd.setTextSize(2);
  M5.Lcd.setTextColor(WHITE);
  M5.Lcd.setCursor(1, 0);

  M5.Lcd.println("MagicFocusEF");
  M5.Lcd.setTextSize(1);
  M5.Lcd.println(" ");
  M5.Lcd.setTextColor(ORANGE);
  M5.Lcd.print("Canon EF 24-70 2.8 IS L");
  M5.Lcd.setTextSize(1);
  ...
  M5.Lcd.setTextColor(WHITE);
    ...
    for (int i = 0; i <= 20; i++) {
     ...
     /* and reversing a little bit not to directly hit the end of the range again */
     for (int i = 0; i <= 5; i++) {
          uint16_t focusVal = -2;
          setFocus(focusVal); 
          delay(10);
...
//    int PLAY_reading = digitalRead(PLAY_BUTTON_PIN);
     ...
     M5.Lcd.fillRect(10, 50, 70, 15, BLACK); 
     M5.Lcd.setCursor(10, 50);
     ...
     Serial.println(focusVal_current);
